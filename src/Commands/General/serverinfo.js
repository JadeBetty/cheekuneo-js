...
export default {
  ...
  run: async (client, interaction) => {
    ...
    const totalThreads = interaction.guild.channels.cache.filter(
      (channel) =>
    ...
    const totalChannels =
      ...
      totalCategories -
      totalThreads;
    const numChannels = interaction.guild.channels.cache.filter(
      (channel) => channel.type === ChannelType.GuildText
    ).size;
    const numVoicechannel = interaction.guild.channels.cache.filter(
      (channel) => channel.type === ChannelType.GuildVoice
    ).size;
    const numForumChannel = interaction.guild.channels.cache.filter(
